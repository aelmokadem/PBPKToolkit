...
#' Test for unrealistic hematocrit value
#'
#' Takes in a named list of Kp values and returns an error if a negative value occurs
#'
...
test_Ht <- function(Ht, BP=1){
  ...
  if((as.numeric(Ht) + as.numeric(BP)) < 1.0) stop("Chosen Ht and BP combination is unrealistic")
}
...
#' Calculate partition coefficient for the "other" compartment
#'
#' Takes in Kp values for all compartments and returns the partition coefficient of the "other" compartment calculated as the average of the non adipose Kp values
#'
#' @param Kp Named list of tissue:plasma partition coefficients
#' @return Partition coefficient for the "other" compartment
#' @keywords internal
calcKpot <- function(Kp){
  Kp_nonAdipose <- Kp[!grepl("ad", names(Kp))]
  Kpot <- mean(as.numeric(Kp_nonAdipose))
  return(Kpot)
}
...
#' @param Vt Named list of tissue volumes generated by genInd
#' @param Kpot Partition coefficient for the "other" compartment
...
scaleKp <- function(Kp, Vss, BP, Vt, Kpot, Ht=0.45){
  ...
  Kp2 <- Kp
  if(is.null(Kpot)) Kpot <- calcKpot(Kp)
  Kp2$Kpot <- Kpot
  ...
  scale <- (Vss - (Vp + EP*Ve)) / (sum(as.numeric(Kp2) * as.numeric(vols3)))

  Kp3 <- lapply(Kp, function(x) scale*x)
...
#' @keywords internal
test_genIndInput <- function(age, bw_targ, ht_targ, bmi_targ){
  age_range <- range(nhanesData$AGE_YR)
  if(age < age_range[1] | age > age_range[2]) stop("Input age is out of range of the internal database")
  ...
  if(!is.null(bw_targ) && !is.null(ht_targ) && !is.null(bmi_targ)) warning("bmi_targ will be calculated from bw_targ and bmi_targ; input bmi_targ value will be ignored")
}
